...
function renderWorkForm(){
  ...
  works.forEach((w,i)=>{h+=`<div class="item-card">
    ...
    <input type="text" list="kb-sched-categories" value="${w.t.replace(/"/g,'&quot;')}" 
      ...
      onblur="setTimeout(() => { if (!this.value) { this.value = this.getAttribute('data-old') || ''; } else { works[${i}].t = this.value; draw(); if(typeof window.translateWorkTitleDirect === 'function') { window.translateWorkTitleDirect(${i}, this.value); } } }, 250);"
    ...
    <textarea id="works-desc-${i}" oninput="works[${i}].d=this.value;drawDebounced()" onblur="if(typeof window.translateWorkDescDirect === 'function') { window.translateWorkDescDirect(${i}, this.value); }">${w.d}</textarea>
...
el('f_sched_pdf').onchange=function(){
...
async function fetchWeatherFromGPS() {
  ...
  try {
    ...
    for(let i=0; i<times.length; i++) {
      ...
      // Khung giờ 6h - 21h
      if(hour >= 6 && hour <= 21) {
        if(p > 0.5) { // Ngưỡng ảnh hưởng > 0.5mm
          rainHoursCount++;
    ...
    let note = "";
    if(rainHoursCount > 0) {
      note = `Mưa các khung giờ: ${rainDetails.join(', ')}. Đánh giá: Mưa có ảnh hưởng đến công tác thi công ngoài trời.`;
    } else {
...
function toggleFormLock(isLocked) {
  try {
    ...
    // 1. Khóa tất cả input, select, textarea ngoại trừ ô ngày báo cáo
...
function updateActionButtons() {
  ...
  html += `<button class="act" type="button" style="background:var(--hp-primary); box-shadow:0 4px 12px rgba(96,187,70,0.25)" onclick="exportPNG169()">📸 Xuất ảnh</button>`;
...
async function saveReportData(targetStatus) {
  try {
    ...
    window.parent.postMessage({ type: 'DAILY_REPORT_SAVED' }, '*');
...
if (localStorage.getItem('meta_dark_mode') === 'true') {
...
window.addEventListener('message', async (e) => {
  ...
  if (e.data.type === 'SYNC_KB') {
    ...
    (async () => {
      try {
        if (window.parent && window.parent.DataService && window.parent.CUR.project) {
          const projs = await window.parent.DataService.listProjects();
          const curProj = projs.find(p => p.id === window.parent.CUR.project);
          if (curProj) {
...
// Yêu cầu KB ngay khi iframe load — tránh race condition với 1500ms timeout
window.parent.postMessage({ type: 'REQUEST_KB_SYNC' }, '*');
