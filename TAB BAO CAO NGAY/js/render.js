...
async function exportPNG169() {
  try {
    ...
    // Đợi mọi <img> trong container tải xong (ảnh Storage tải qua mạng); lỗi/timeout vẫn tiếp để không treo.
    function waitAllImages(container) {
      ...
      return Promise.all(imgs.map(img => {
        ...
        return new Promise(res => {
          ...
          setTimeout(res, 10000); // chốt an toàn 10s/ảnh
        });
      }));
    }
