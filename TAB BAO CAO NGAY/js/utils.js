...
function renderUnitForm(){
  ...
  const activeKb = kb.filter(c => c.status !== 'inactive');
  let datalistHtml = `<datalist id="kb-contractors-list">`;
  activeKb.forEach(c => {
    datalistHtml += `<option value="${c.name}"></option>`;
  });
  ...
  units.forEach((u,i)=>{
    ...
    <div style="flex:2">
      ...
      <input type="text" list="kb-contractors-list" value="${(u.name||'').replace(/"/g,'&quot;')}" 
        onfocus="this.setAttribute('data-old', this.value); this.value = '';" 
        onblur="setTimeout(() => { if (!this.value) { this.value = this.getAttribute('data-old') || ''; } else { units[${i}].name = this.value; draw(); } }, 250);"
        oninput="units[${i}].name=this.value;drawDebounced()"
        placeholder="Nhập hoặc chọn đơn vị...">
    </div>
    <div style="flex:0 0 90px"><label>Số người</label><input type="number" value="${u.n}" oninput="units[${i}].n=parseInt(this.value)||0;recomputeTotal()"></div>
    </div>
    <button class="delbtn" type="button" onclick="units.splice(${i},1);renderUnitForm();recomputeTotal()">Xóa</button></div>`
  });
  el('unit-list').innerHTML=h + datalistHtml;
